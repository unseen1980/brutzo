...
same node graph — that is the harness requirement. `app` `#/harness` loads
`/harness/clips/manifest.json`, plays each clip through the same graph as the microphone
(buffer source → channel auto-select → analysers → YIN), and reports pass/fail with cents
error per clip. The current clips are **synthetic** (generated by
`harness/generate-clips.mjs`); the real Katana-USB recordings get added to the same folder
and manifest. Phase 0 exit criterion: ≥ 95% note accuracy on the clip library.
